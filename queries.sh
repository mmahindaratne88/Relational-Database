...
echo "$($PSQL "SELECT name FROM teams WHERE team_id = (SELECT winner_id FROM games WHERE year = 2018 LIMIT 1)")"
...
echo "$($PSQL "SELECT yeat, name FROM teams INNER JOIN games ON teams.team_id = games.winner_id AND round='Final'")"
